...
empty-folder-susceptible-hidden = Susceptible samples are hidden
...
show-susceptible-samples = Show susceptible samples
show-as-samples = Show as samples
...
tb-profiler-config-error = TB Profiler Configuration Error
tb-profiler-config-error-body = Pair suffixes are not configured. Please set them in TB Profiler settings.
