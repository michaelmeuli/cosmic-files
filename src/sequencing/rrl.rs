//! 23S rRNA (rrl) macrolide resistance calling for *M. abscessus*, *M. avium*, and
//! *M. intracellulare*.
//!
//! Mirrors [`erm41`](super::erm41)'s two-line-of-evidence structure: a fixed diagnostic position
//! ([`RrlPosition2058_2059`], the wildtype-vs-mutant call at *E. coli* positions 2058/2059) plus
//! a broader catalogue of resistance-conferring SNPs ([`RrlSnpCall`]) parsed from ntm-db's
//! per-species `variants.csv`, combined in [`is_susceptible_rrl`].
...
/// The call at rrl positions 2058/2059 (*E. coli* numbering), the two best-characterized
/// macrolide resistance sites in 23S rRNA.
#[derive(Debug, Clone, Copy, PartialEq, Serialize, Deserialize)]
pub enum RrlPosition2058_2059 {
    /// Both positions match wildtype (A2058 and A2059) — susceptible.
    SusceptibleWildtype,
    /// Either position carries a non-wildtype base — resistance-conferring.
    ResistanceConferringMutation,
    /// Anchor sequence not found in the read, so the bases couldn't be located.
    Undetermined,
}
...
pub struct RrlSusceptibilityCalls {
    /// Position 2058/2059 call, or `None` if the anchor wasn't found in any read.
    ...
    /// Combined verdict from [`is_susceptible_rrl`]: position 2058/2059 overridden by any
    /// observed resistance SNP.
    pub is_susceptible: Option<bool>,
    /// Verdict from [`is_susceptible_rrl_by_snp_calls_rare`]: catches resistance mutations
    /// outside 2058/2059 even when that position itself is wildtype.
...
/// Parses rrl resistance SNPs (`gene == "rrl"`, `type == "drug_resistance"`) out of an ntm-db
/// `variants.csv`, keyed by **0-based nucleotide position** in the rrl reference sequence.
/// Mutations are expected in `n.<pos><wt>><alt>` HGVS nucleotide notation (e.g. `n.2058A>G`);
...
impl RrlSnpCall {
    /// Human-readable tag, e.g. `A2059G (clarithromycin, E.coli: A2059G)` for a resistance alt,
    /// or `A2059A (E.coli: A2059A)`/`A2059?` otherwise — always includes the *E. coli*-numbered
    /// equivalent when known, since that's the numbering most macrolide-resistance literature
    /// uses.
...
/// Identifies the closest-matching mycobacterial species for an rrl read, and attaches
/// position-2058/2059 and resistance-SNP calls to hits whose reference came from ntm-db (i.e.
/// whose accession contains `:`, marking a full-gene ntm-db sequence rather than an NCBI
/// partial sequence — see `extract_ntm_db_sequences()` in `build.rs`).
///
/// References shorter than [`super::MIN_RRL_REF_LEN`] are excluded up front, since rrl partial
/// sequences below that length are too short to distinguish species reliably.
///
/// The reference database is generated at build time by filtering NCBI Nucleotide for
/// `Mycobacteriaceae[Organism] AND (23S ribosomal RNA[Title] OR rrl[Gene Name]) AND
/// 1000:3200[SLEN] AND type_material[Filter]` (`fetch_myco_sequences()` in `build.rs`), plus the
/// rrl sequences found in [ntm-db](https://github.com/pathogen-profiler/ntm-db)
/// (`extract_ntm_db_sequences()` in `build.rs`).
