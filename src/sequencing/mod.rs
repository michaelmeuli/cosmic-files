...
pub use rrs::identify_species_16s;
...
pub fn identify_species_erm41(query: &[u8]) -> Vec<SeqIdHit> {
    rrs::identify_species(query, REF_MYCO_ERM41)
}
...
pub fn identify_species_rpob(query: &[u8]) -> Vec<SeqIdHit> {
    rrs::identify_species(query, REF_MYCO_RPOB)
}
...
pub fn identify_species_hsp65(query: &[u8]) -> Vec<SeqIdHit> {
    let query = trim_hsp65_primers(query);
    rrs::identify_species(&query, REF_MYCO_HSP65)
}
...
pub fn identify_species_23s_ntm(query: &[u8]) -> Vec<SeqIdHit> {
    let query = trim_start_end(query, RRL_FWD_START, RRL_FWD_END);
    rrs::identify_species(query, REF_MYCO_RRL)
}
