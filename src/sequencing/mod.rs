...
use crate::{operation::recursive::Op, sequencing::rrl::rrlPosition_2057_2058};
...
/// Function doesn't have an effect as species_hits is only used in preview_16s()  
/// Database was generated by filtering NCBI Nucleotide for Mycobacteriaceae[Organism] AND erm(41)[Gene Name] AND 400:3000[SLEN] (via fetch_myco_sequences() in build.rs).  
/// Added to this database are the erm(41) sequences found in [ntm-db](https://github.com/pathogen-profiler/ntm-db) (via extract_ntm_db_sequences() in build.rs).  
pub fn identify_species_erm41(query: &[u8]) -> Vec<SpeciesHit> {
    identify_species(query, REF_MYCO_ERM41)
}

/// Function doesn't have an effect as species_hits is only used in preview_16s()  
/// Database was generated by filtering NCBI Nucleotide for Mycobacteriaceae[Organism] AND rpoB[Gene Name] AND 400:3000[SLEN] AND type_material[Filter] (via fetch_myco_sequences() in build.rs).  
/// Added to this database are the rpoB sequences found in [ntm-db](https://github.com/pathogen-profiler/ntm-db) (via extract_ntm_db_sequences() in build.rs).  
